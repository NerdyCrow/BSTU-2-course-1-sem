...
namespace Log
{
	LOG Log::getlog(wchar_t logfile[])
	{
		...
		if (!result.stream->is_open()) throw ERROR_THROW(112);
	...
	{
		if (log.stream == nullptr || !log.stream->is_open()) std::cout << "Ошибка " << error.id << ": " << error.message << ", строка " << error.inext.line << ", позиция " << error.inext.col << std::endl;
		else *log.stream << "Ошибка " << error.id << ": " << error.message << ", строка " << error.inext.line << ", позиция " << error.inext.col << std::endl;
	}
